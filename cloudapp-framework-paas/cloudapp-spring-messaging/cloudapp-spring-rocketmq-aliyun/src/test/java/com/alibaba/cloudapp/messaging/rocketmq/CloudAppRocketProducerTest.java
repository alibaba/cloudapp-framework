...
public class CloudAppRocketProducerTest {
    ...
    public void setUp() throws Exception {
        ...
        properties.setCompressMsgBodyOverHowMuch(1);
        properties.setMaxMessageSize(1);
        properties.setRetryTimesWhenSendFailed(1);
        properties.setEnableMsgTrace(true);
        properties.setName("produce");
        properties.setNameServer("localhost:9876");
        properties.setNamespace("namespace");
        properties.setGroup("group");
        properties.setPassword("password");
        properties.setUseTLS(true);
        properties.setRetryNextServer(true);
        properties.setDefault(true);
        properties.setUsername("username");
        
        producer = new CloudAppRocketProducer(properties);
        
        Field field = CloudAppRocketProducer.class.getDeclaredField("producer");
        ...
        sendResult.setQueueOffset(1);
        sendResult.setTransactionId("transactionId");
        sendResult.setOffsetMsgId("offsetMsgId");
        ...
        doReturn(sendResult).when(mockProducer).send(any(Message.class));
        doNothing().when(mockProducer).send(any(Message.class),
                                    any(SendCallback.class));
        doNothing().when(mockProducer).shutdown();
    }
    ...
    public void testRefresh() throws MQClientException {
        ...
        assertNotEquals(mockProducer, producer.getDelegatingProducer());
    }
