...
public class RocketProducerProperties {
    ...
    public Properties toProperties() {
        ...
        if(username != null) {
            properties.put(PropertyKeyConst.AccessKey, username);
        }
        if(password != null) {
            properties.put(PropertyKeyConst.SecretKey, password);
        }
        properties.put(PropertyKeyConst.SendMsgTimeoutMillis, sendTimeout * 1000);
        if(nameServer != null) {
            properties.put(PropertyKeyConst.NAMESRV_ADDR, nameServer);
        }
        if(name != null) {
            properties.put(PropertyKeyConst.InstanceName, name);
        }
        if(namespace !=  null) {
            properties.put(PropertyKeyConst.INSTANCE_ID, namespace);
        }
        properties.put(PropertyKeyConst.isVipChannelEnabled, vipChannelEnable);
        return properties;
    }
}
